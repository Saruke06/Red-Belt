...
std::string generateRandomWord(int length) {
    ...
    static std::uniform_int_distribution<> distribution(0, sizeof(alphabet) - 2);
...
std::vector<std::string> UniqueWords(int numWords, int wordLength) {
	std::set<std::string> uniqueWords;

	// Generate a set of unique words
	while (uniqueWords.size() < numWords) {
	    uniqueWords.insert(generateRandomWord(wordLength));
	}

	// Convert the set of unique words to a vector
	return {uniqueWords.begin(), uniqueWords.end()};

}


std::vector<std::string> generateVector(int numRows, int numWords, const vector<string>& words) {
...
void TestTime() {
	const int documents_size = 50'000;
	const int document_size = 1'000;
	const int query_input_size = 500'000;
	const int query_size = 10;
	const int different_words_number = 10'000;
	const int word_max_length = 100;

	vector<string> uniqueWordsVector = UniqueWords(different_words_number, word_max_length);

	vector<string> docs = generateVector(documents_size, document_size, uniqueWordsVector);
	vector<string> queries = generateVector(query_input_size, query_size, uniqueWordsVector);

//	for (auto s : Head(docs, 10)) {
//		cout << s << endl;
//	}
//
//	for (auto s : Head(queries, 10)) {
//		cout << s << endl;
//	}
