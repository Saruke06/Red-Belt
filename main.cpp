...
int main() {
  ifstream stream1("search_result.txt");
  ifstream stream2("queri_input.txt");
  ifstream stream3("output.txt");
  ostringstream os;
  {
      istringstream ss("london is the capital of great britain\ni am traveling down the river");
      istringstream ss1("london\nthe");
      SearchServer srv(ss);
      srv.AddQueriesStream(ss1,os);
  }
  cout << os.str() << endl;
