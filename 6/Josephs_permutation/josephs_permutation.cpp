...
void MakeJosephusPermutation(RandomIt first, RandomIt last, uint32_t step_size) {
  ...
  typename list<typename RandomIt::value_type>::iterator cur_pos = pool.begin();
  while (!pool.empty()) {
    *(first++) = move(*cur_pos);
    if (pool.size() == 1) break;

    auto next_pos = LoopIterator(pool, next(cur_pos));
    pool.erase(cur_pos);
    cur_pos = next_pos;

    for (uint32_t step_ind = 1; step_ind < step_size; ++step_ind) {
